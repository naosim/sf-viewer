...
- その他のTSVファイル - queryJobs で指定したオブジェクトのデータ

### TSVの列順序

TSVを出力する際、識別しやすい列を優先して左側に配置します。優先度は以下の通りです:

1. **Id** - レコードID（最優先）
2. **Name** - 名前
3. **Title** - タイトル

例えば、Product2 オブジェクトの場合、Id と Name 列が左側に移動します。
...
HTMLテンプレートは `src/tsv-doc/templates/viewer.html` に配置されています。プレースホルダー:
...
viewer.html にはデータフィルタリング機能が組み込まれています。
...
| 括弧 | `(A == 1 OR A == 2) AND B == 3` | グループ化 |

#### 使用例

```
ProductCode IN ('GC1060', 'GC1020')
ProductCode == 'GC1060' AND IsActive == true
Name LIKE '%Diesel%'
Description IS NULL
(Status == 'Active' OR Status == 'Pending') AND IsActive == true
```
